...
import com.ilovefish.caviarmvvm.network.interceptor.logging.DefaultFormatPrinter
import com.ilovefish.caviarmvvm.network.interceptor.logging.FormatPrinter
import com.ilovefish.caviarmvvm.network.interceptor.logging.LogInterceptor
import com.ilovefish.caviarmvvm.util.CharacterHandler
import com.ilovefish.caviarmvvm.util.UrlEncoderUtils
import com.ilovefish.caviarmvvm.util.ZipHelper
import okhttp3.*
import okio.Buffer
...
class LogInterceptor: Interceptor {
    ...
    private val printLevel = LogInterceptor.Level.ALL
    override fun intercept(chain: Interceptor.Chain): Response {
        val request = chain.request()
        val logRequest =
            printLevel == LogInterceptor.Level.ALL || printLevel != LogInterceptor.Level.NONE && printLevel == LogInterceptor.Level.REQUEST
        if (logRequest) {
            //打印请求信息
            if (request.body() != null && LogInterceptor.isParseable(
                    request.body()!!.contentType()
                )
            ) {
                mPrinter.printJsonRequest(request, LogInterceptor.parseParams(request))
            } else {
                mPrinter.printFileRequest(request)
            }
        }
        val logResponse =
            printLevel == LogInterceptor.Level.ALL || printLevel != LogInterceptor.Level.NONE && printLevel == LogInterceptor.Level.RESPONSE
        ...
        if (responseBody != null && LogInterceptor.isParseable(responseBody.contentType())) {
